...
func RedisCache(rdb *redis.Client, ttl time.Duration) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		val, _, shared := sfGroup.Do(cacheKey, func() (interface{}, error) {
			...
			if bw.statusCode == http.StatusOK && bw.body.Len() > 0 {
				responseBytes := make([]byte, bw.body.Len())
				copy(responseBytes, bw.body.Bytes())
				...
				if errPipe != nil {
					...
					config.Log.Info("Cache MISS, veri Redis'e kaydedildi", zap.String("key", cacheKey))
				}

				bw.flush()
				return &cacheResult{data: responseBytes, statusCode: http.StatusOK}, nil
			}

			config.Log.Warn("Controller hata döndü veya DB timeout, Redis fallback deneniyor",
				zap.Int("status", bw.statusCode), zap.String("key", cacheKey))
			...
			if staleErr == nil && len(staleData) > 0 {
				...
				bw.flush()
				return &cacheResult{data: staleData, statusCode: http.StatusOK}, nil
			}
			...
			copy(responseBytes, bw.body.Bytes())
			bw.flush()
			return &cacheResult{data: responseBytes, statusCode: bw.statusCode}, nil
		})

		if shared {
			cr := val.(*cacheResult)
			config.Log.Info("Singleflight paylaşımlı yanıt sunuldu", zap.String("key", cacheKey))
			c.Header("X-Cache", "singleflight")
			c.Data(cr.statusCode, "application/json; charset=utf-8", cr.data)
			c.Abort()
		}
	}
}
